...
async def preload_characters(context: CallbackContext) -> None:
    ...
    try:
        ...
        all_characters = await collection.find({'id': {'$gte': '01', '$lte': '4500'}}).to_list(length=None)
...
async def message_counter(update: Update, context: CallbackContext) -> None:
    ...
    async with lock:
        ...
        if chat_id not in total_message_counts:
            ...
            valentine_spawn_thresholds[chat_id] = random.randint(700, 1700)
...
async def send_image(update: Update, context: CallbackContext) -> None:
    ...
    spawn_counts = {
        ...
        '💝 Valentine': 1, 
        '⚪️ Common': 5,  # Start spawning Valentine characters
        '🎃 Halloween': 1,
        '🟡 Legendary': 5, 
        '⚪️ Common': 5,
        '🎄 Christmas Special': 1,
